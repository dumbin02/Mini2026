...
void setPWM(int pwmA, int pwmB){
  if(pwmA >= 0){
    ...
    digitalWrite(MOTOR_A1, LOW);
    digitalWrite(MOTOR_A2, HIGH); 
    if(MM.velocidadMotorA >= -35) MM.direccionMotorA = 1;   
  } else{
    pwmA = -(pwmA);
    ledcWrite(PWM_A, pwmA);
    digitalWrite(MOTOR_A1, HIGH);
    digitalWrite(MOTOR_A2, LOW);
    if(MM.velocidadMotorA <= 35) MM.direccionMotorA = 0;
  }
  if(pwmB >= 0){
    ledcWrite(PWM_B, pwmB);
    digitalWrite(MOTOR_B1, HIGH);
    digitalWrite(MOTOR_B2, LOW);
    if(MM.velocidadMotorB >= -35) MM.direccionMotorB = 1;
  }else{
    pwmB = -(pwmB);
    ledcWrite(PWM_B, pwmB);
    digitalWrite(MOTOR_B1, LOW);
    digitalWrite(MOTOR_B2, HIGH);
    if(MM.velocidadMotorB <= 35) MM.direccionMotorB = 0;
  }
